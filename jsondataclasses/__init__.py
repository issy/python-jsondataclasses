...
def parse_field(
    ...
    json_key: str,
    field_type: type[_T],
    field_parser: Optional[Callable[[Any], _T]],
    default_value: Any,
) -> _T:
    type_of_type = type(field_type)

    if (
        json_key not in data
        and type_of_type is not typing._UnionGenericAlias
    ...
    elif type_of_type is typing._UnionGenericAlias and isinstance(
        None, field_type.__args__[1]
    ):  # Parse Optional[Any]
        return (
            parse_field(
                data, json_key, field_type.__args__[0], field_parser, default_value
            )
            if json_key in data
            else None
        )
        ...
        parsed_value = parse_field(
            data, json_key, type(field_type.__args__[0]), field_parser, default_value
        )
        if parsed_value not in field_type.__args__:
            raise ValueError(
                f"{getattr(data, json_key, default_value)} not of literal {field_type.__args__!r}"
            )
        return parsed_value
    ...
    def __init__(self, data: dict):
        for key, field_meta in fields.items():
            ...
            setattr(
                ...
                key,
                parse_field(
                    data, field_meta[0], field_type, field_meta[1], field_meta[2]
                ),
            )
